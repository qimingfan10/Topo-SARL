...
def train(args):
    ...
    print(f"✓ 奖励配置:")
    print(f"  - 使用GT: {config['reward']['use_gt']}")
    print(f"  - IoU权重: {config['reward']['iou_weight']}")
    print(f"  - clDice权重: {config['reward']['cldice_weight']}")
    print(f"  - Vesselness权重: {config['reward']['vesselness_weight']}")
    print()
    ...
    # 保存最终模型
    final_model_path = save_dir / 'ppo_enhanced_final.zip'
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='增强训练脚本（阶段 A）')
    parser.add_argument(
        '--config',
        type=str,
