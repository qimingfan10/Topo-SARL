...
class RL4Seg3DPredictor:
    ...
    def run_system(cfg: DictConfig) -> Tuple[dict, dict]:
        ...
        if torch.load(cfg.ckpt_path, weights_only=False).get("pytorch-lightning_version", None):
            trainer.predict(model=model, dataloaders=dataloader, ckpt_path=cfg.ckpt_path)
        else:
            model.net.load_state_dict(torch.load(cfg.ckpt_path, weights_only=False))
