...
class RL3dDataset(Dataset):
    def __init__(self,
        ...
        # 数据增强配置
        self.use_augmentation = use_augmentation and not self.test and not self.val
        if self.use_augmentation:
            print("✅ 数据增强已启用（仅训练集）")
            # 定义数据增强变换
            self.transform = tio.Compose([
                # 空间变换
                tio.RandomFlip(axes=(0, 1), p=0.5),  # 50%概率水平/垂直翻转
                tio.RandomAffine(
                    scales=(0.9, 1.1),        # 缩放±10%
                    degrees=15,               # 旋转±15度
                    translation=5,            # 平移±5像素
                    p=0.5                     # 50%概率应用
                ),
                
                # 强度变换
                tio.RandomBlur(std=(0, 1.5), p=0.3),        # 30%概率随机模糊
                tio.RandomNoise(std=(0, 0.05), p=0.3),      # 30%概率添加噪声
                tio.RandomGamma(log_gamma=(-0.2, 0.2), p=0.3),  # 30%概率Gamma变换
                
                # 对比度和亮度
                tio.RandomBiasField(coefficients=0.3, p=0.2),  # 20%概率偏置场
            ])
        else:
            self.transform = None
    ...
    def __getitem__(self, idx):
        ...
        if debug_on:
            try:
                logger.debug(f"[rank{rank}] __getitem__ idx={idx} sub_path={sub_path} raw_img_shape={img.shape} raw_mask_shape={mask.shape}")
            except Exception:
                pass
        ...
        # Ensure img is 4D (C, H, W, D) for TorchIO
        if img.ndim == 3:
            # (H,W,D) -> (C=1,H,W,D)
            img_tensor = np.expand_dims(img, 0)
        elif img.ndim == 4:
            # Observed nifti format: (H, W, 1, T). We want (C=1, H, W, D=T)
            # Generalized: choose channel axis as argmin (often 1), depth as last axis
            ch_axis = int(np.argmin(img.shape))
            if ch_axis == 0:
                img_tensor = np.transpose(img, (0, 1, 2, 3))  # unlikely
            elif ch_axis == 1:
                img_tensor = np.transpose(img, (1, 0, 2, 3))
            elif ch_axis == 2:
                # common case (H,W,1,T) -> (1,H,W,T)
                img_tensor = np.transpose(img, (2, 0, 1, 3))
            else:
                # fallback: last axis is channel (rare)
                img_tensor = np.transpose(img, (3, 0, 1, 2))
        else:
            raise ValueError(f"Unexpected image dimensions: {img.ndim}D. Expected 3D or 4D.")
        resampled = transform(tio.ScalarImage(tensor=img_tensor, affine=img_nifti.affine))

        if debug_on:
            try:
                logger.debug(f"[rank{rank}] after_resample img_tensor_shape={img_tensor.shape} resampled_tensor_shape={resampled.tensor.shape}")
            except Exception:
                pass
        ...
        if debug_on:
            try:
                logger.debug(f"[rank{rank}] after_crop img_shape={img.shape}")
            except Exception:
                pass
        # Ensure mask is 4D (C, H, W, D) for TorchIO
        if mask.ndim == 3:
            mask_tensor = np.expand_dims(mask, 0)
        elif mask.ndim == 4:
            # align with image transposition: set channel as the smallest axis
            ch_axis = int(np.argmin(mask.shape))
            if ch_axis == 2:
                mask_tensor = np.transpose(mask, (2, 0, 1, 3))
            elif ch_axis == 0:
                mask_tensor = np.transpose(mask, (0, 1, 2, 3))
            elif ch_axis == 1:
                mask_tensor = np.transpose(mask, (1, 0, 2, 3))
            else:
                mask_tensor = np.transpose(mask, (3, 0, 1, 2))
        else:
            raise ValueError(f"Unexpected mask dimensions: {mask.ndim}D. Expected 3D or 4D.")
        mask = croporpad(transform(tio.LabelMap(tensor=mask_tensor,
                                                affine=img_nifti.affine))).tensor.squeeze(0)
        
        # Ensure approx_gt is 4D (C, H, W, D) for TorchIO
        if approx_gt.ndim == 3:
            approx_gt_tensor = np.expand_dims(approx_gt, 0)
        elif approx_gt.ndim == 4:
            ch_axis = int(np.argmin(approx_gt.shape))
            if ch_axis == 2:
                approx_gt_tensor = np.transpose(approx_gt, (2, 0, 1, 3))
            elif ch_axis == 0:
                approx_gt_tensor = np.transpose(approx_gt, (0, 1, 2, 3))
            elif ch_axis == 1:
                approx_gt_tensor = np.transpose(approx_gt, (1, 0, 2, 3))
            else:
                approx_gt_tensor = np.transpose(approx_gt, (3, 0, 1, 2))
        else:
            raise ValueError(f"Unexpected approx_gt dimensions: {approx_gt.ndim}D. Expected 3D or 4D.")
        approx_gt = croporpad(transform(tio.LabelMap(tensor=approx_gt_tensor,
                                                     affine=img_nifti.affine))).tensor.squeeze(0)

        if debug_on:
            try:
                logger.debug(f"[rank{rank}] after_mask_gt mask_shape={mask.shape} approx_gt_shape={approx_gt.shape}")
            except Exception:
                pass
        ...
        if not self.test:
            if self.max_window_len:
                ...
                if debug_on:
                    try:
                        logger.debug(f"[rank{rank}] stacked windows img_shape={img.shape} mask_shape={mask.shape} approx_gt_shape={approx_gt.shape}")
                    except Exception:
                        pass
            else:
        ...
        if debug_on:
            try:
                logger.debug(f"[rank{rank}] final batch img_shape={img.shape} mask_shape={mask.shape} approx_gt_shape={approx_gt.shape}")
            except Exception:
                pass
        ...
        if self.use_augmentation and self.transform is not None:
            ...
            augmented_imgs = []
            augmented_masks = []
            augmented_approx_gts = []
            
            for b_idx in range(img.shape[0]):
                # 创建TorchIO Subject
                subject = tio.Subject(
                    image=tio.ScalarImage(tensor=img[b_idx]),
                    mask=tio.LabelMap(tensor=mask[b_idx].unsqueeze(0) if mask[b_idx].ndim == 3 else mask[b_idx]),
                    approx_gt=tio.LabelMap(tensor=approx_gt[b_idx].unsqueeze(0) if approx_gt[b_idx].ndim == 3 else approx_gt[b_idx])
                )
                
                # 应用变换
                try:
                    augmented = self.transform(subject)
                    augmented_imgs.append(augmented['image'].data)
                    augmented_masks.append(augmented['mask'].data.squeeze(0) if augmented['mask'].data.shape[0] == 1 else augmented['mask'].data)
                    augmented_approx_gts.append(augmented['approx_gt'].data.squeeze(0) if augmented['approx_gt'].data.shape[0] == 1 else augmented['approx_gt'].data)
                except Exception as e:
                    # 如果增强失败，使用原始数据
                    if debug_on:
                        logger.debug(f"[rank{rank}] Augmentation failed for sample {b_idx}: {e}")
                    augmented_imgs.append(img[b_idx])
                    augmented_masks.append(mask[b_idx])
                    augmented_approx_gts.append(approx_gt[b_idx])
            
            # 重新堆叠
            img = torch.stack(augmented_imgs)
            mask = torch.stack(augmented_masks)
            approx_gt = torch.stack(augmented_approx_gts)

        # 维度规范：保持 5D，不展平时间维，统一为 [batch, channels, H, W, D]
        # 上游 3D 模型会在前向时重排为 Conv3d 期望的 [batch, channels, D, H, W]
...
class RL3dDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        if 'study' not in self.df.columns:
            self.df = self.df.reset_index()
            # Rename the index column to 'study' if it was named something else
            if self.df.columns[0] != 'study':
                self.df = self.df.rename(columns={self.df.columns[0]: 'study'})
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if self.hparams.max_image_area:
            if not {'H', 'W'}.issubset(self.df.columns):
                raise ValueError("MISSING KEY FOR MAX_IMAGE_AREA!")
        ...
        if stage == "test" or stage is None:
            # 若无显式 test split，则回退：pred -> val -> train
            test_idx_to_use = self.test_idx
            if not test_idx_to_use:
                print("Warning: No 'test' split found. Falling back to alternative splits for testing.")
                if hasattr(self, 'pred_idx') and len(self.pred_idx) > 0:
                    test_idx_to_use = self.pred_idx
                    print(f"Using 'pred' split for testing ({len(test_idx_to_use)} samples)")
                elif hasattr(self, 'val_idx') and len(self.val_idx) > 0:
                    test_idx_to_use = self.val_idx
                    print(f"Using 'val' split for testing ({len(test_idx_to_use)} samples)")
                elif hasattr(self, 'train_idx') and len(self.train_idx) > 0:
                    # Use a subset of train for testing (e.g., first 10% or max 5 samples)
                    test_idx_to_use = self.train_idx[:min(max(1, len(self.train_idx) // 10), 5)]
                    print(f"Using subset of 'train' split for testing ({len(test_idx_to_use)} samples)")
            
            self.data_test = self.hparams.dataset(self.df.loc[test_idx_to_use],
                                                    data_path=self.data_path,
                                                    test=True,
                                                    common_spacing=common_spacing,
                                                    shape_divisible_by=list(self.hparams.shape_divisible_by),
        ...
        if stage == "predict":
            # 若无显式 pred split，则回退：test -> val -> train
            if not self.pred_idx:
                if hasattr(self, 'test_idx') and len(self.test_idx) > 0:
                    self.pred_idx = self.test_idx
                elif hasattr(self, 'val_idx') and len(self.val_idx) > 0:
                    self.pred_idx = self.val_idx
                else:
                    self.pred_idx = self.train_idx
